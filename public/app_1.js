...
window.addEventListener('load', function(){
    ...
    // document.getElementById('subDate-dropdown').addEventListener('change', ()=> {
    //     let selectedmonth = document.getElementsByName('subDate').select;
    ...
    //Detect when dropdown has changed
        ...
        //creating the object
        let obj = {"month" : selectedMonthNumber};

        //stringifying the oject
        let jsonData = JSON.stringify(obj);


        //fetch to route selectedmonth
        fetch('/selectedmonth', {
            method: 'POST',
            headers: {
        ...
        document.getElementById('get-month').addEventListener('click',()=>{
            fetch('/getMonth')
            .then(resp=>resp.json())
            .then(data => {
                ...
                for(let i=0; i<data.data.length; i++) {
                    let string = data.data[i].date + " : " + data.data[i].month.month;
                let elt = document.createElement('p');
                elt.innerHTML = string;
                document.getElementById('month-info').appendChild(elt);
